...
def load_dict(use_trie: bool) -> Any:
  ...
  pickle_filename = f"{os.environ.get('HOME')}/wordfinder/android/{bn}_words.pickle" 
  if use_trie and os.path.exists(pickle_filename):
    with open(pickle_filename, "rb") as picklefile:
      return pickle.load(picklefile)

  else:
    with open(f"{os.environ.get('HOME')}/wordfinder/android/scrabble_words.txt") as dictfile:
